...
if ~exist('master_fast')
    master_fast=0;
end
if ~exist('patch_average_sub')
    ARG.patch_average_sub=1;
else
    ARG.patch_average_sub=patch_average_sub;
end
...
if ~exist('NVR_threshold')
    ARG.NVR_threshold=1;
else
    ARG.NVR_threshold=NVR_threshold;
end
if ~exist('file_idx')
    ARG.file_idx=1;
else
    ARG.file_idx= file_idx;
end
if ~exist('kernel_size')
    ARG.kernel_size=[4 4 4];
else
    ARG.kernel_size= kernel_size;
end
if ~exist('patch_average')
    ARG.patch_average=1;
else
    ARG.patch_average=patch_average;
end
if ~exist('APPEND_NAME')
    ARG.APPEND_NAME=[];
else
    ARG.APPEND_NAME=APPEND_NAME;
end
if ~exist('NVR2')
    ARG.NVR2=[];
else
...
if ~exist('truncate_length')
    ARG.truncate_length=[];
else
    ARG.truncate_length= truncate_length;
end
if ~exist('restart_list')
    ARG.restart_list=0;
else
    ARG.restart_list= restart_list;
end
if ~exist('soft_thrs')
    ARG.soft_thrs=[] ;
else
    ARG.soft_thrs= soft_thrs;
end
...
if sum(kernel_size)==0  % choose optimal kernel
    ...
    APPEND_NAME=['kernel' num2str(ker)];
    ARG.APPEND_NAME=APPEND_NAME;
end
...
if isempty(filename_out) |1  % process is not complete
 ...
 % return
  ...
  if master==1
      KSP_processed=QQ.KSP_processed;
      if sum(KSP_processed(KSP_processed<2))>0
        pause(60*5);
    end  % hopefully enough time for other processes to finish
   for n1=1:size(QQ,'KSP_processed',2)
       fprintf( [num2str(n1) ' '])
...
try
    ...
    if ARG.soft_thrs~=10;
        NOISE=[];
    end  % empty it unless it is calculated
...
KSP=newname.KSP;clear newname
 ...
 %       MASK(:,:,nsl)= bwmorph( abs(KSP(:,:,nsl,1))>0,'shrink',1);
    ...
    for niter=1:size(KSP,4)
        KSP(:,:,:,niter)=KSP(:,:,:,niter)./(g_factor_map+eps);
    end
  ...
  meanphase=mean(KSP(:,:,:,[1:end-1]),4);
   ...
   for slice=matdim(3):-1:1
      for n=1:size(KSP,4)-1;
         tmp=KSP(:,:,slice,n);
            for ndim=[1:2]
                tmp=ifftshift(ifft(ifftshift( tmp ,ndim),[],ndim),ndim+0);
            end
            ...
            for ndim=[1:2]
                tmp=fftshift(fft(fftshift( tmp ,ndim),[],ndim),ndim+0);
            end
